                     ...
                     <div class="form-floating form-floating-outline">
                        <select name="parent_id" id="parent_id" class="form-select select2">
                           ...
                           @foreach ($parentMenus as $pm)
                              @if ($pm->id != $menu->id)
                                 <option value="{{ $pm->id }}" {{ $menu->parent_id == $pm->id ? 'selected' : '' }}>
                                    {{ $pm->name }}</option>
                              @endif
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-6 mb-4">
                     <div class="form-floating form-floating-outline">
                        <input type="text" name="slug" id="slug"
                           class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug', $menu->slug) }}"
                           required>
                  ...
                  <div class="col-md-3 mb-4">
                     <div class="form-floating form-floating-outline">
                        <input type="number" name="order_no" id="order_no" class="form-control"
                           value="{{ old('order_no', $menu->order_no) }}" required>
